  ...
  //  public popupGameSBO: PopupGameSBO = null;
    ...
    actChangeAvatar() {
        if (!Configs.Login.IsLogin) {
            this.alertDialog.showMsg("អ្នកមិនបានចូលទេ។.");
            return;
        }
    ...
    static DOMAIN = "vietkieu888.com";
    static API_CMD: string = "https://ga.vietkieu888.com/3rd/cmd";
    static API_IBC: string = "https://ga.vietkieu888.com/3rd/ibc";
    static API_SBO: string = "https://ga.vietkieu888.com/3rd/sbo"
    static API_AG: string = "https://ga.vietkieu888.com/3rd/ag";
    static API_EBET: string = "https://ga.vietkieu888.com/3rd/ebet";
    static API_WM: string = "https://ga.vietkieu888.com/3rd/wm";

    //http://localhost:8081/api?c=2021&nn=tuanbigbird&at=1628224022&ip=127.0.0.1&mn=100
    updateConfigGame(domain = "vietkieu888.com") {
        ...
        SlotNetworkClient.getInstance().addListener((data) => {
            ...
            switch (inPacket.getCmdId()) {
                ...
                case cmd.Code.UPDATE_BIGWIN_JACKPOT_SLOT3x3GEM: {
                    let res = new cmd.ResNotifyJackpot(data);
                    this.showJackpotNotify(res, "Lọ Nước Thần");
                    break;
                }
                case cmd.Code.UPDATE_BIGWIN_JACKPOT_MINIPOKER: {
                    let res = new cmd.ResNotifyJackpot(data);
                    this.showJackpotNotify(res, "Mini Mậu Binh");
                    break;
                }
    ...
    showJackpotNotify(res, gameName) {
        ...
        if (res["type"] == 3) {
            ...
            dataNotify["type"] = res["type"] == 3 ? "Jackpot" : "ឈ្នះធំ";
    ...
    showLoading(isShow: boolean, timeOut: number = 15) {
       ...
       // this.loadingIcon.stopAllActions();
        //this.loadingIcon.runAction(cc.repeatForever(cc.rotateBy(1.5, 360)));
         //cc.tween(this.loadingIcon).by(0.1, { scale: 1.2 }).by(0.1, { scale: 0.8 }).by(0.1, { scale: 1.0 }).repeatForever().start();
    }

    showErrLoading(msg?: string) {
        this.showLoading(true, -1);
        this.loadingLabel.string = msg ? msg : "បាត់បង់ការតភ្ជាប់ សូមព្យាយាមម្តងទៀត...";
    }
    ...
    getAvatarSpriteFrame(avatar: string): cc.SpriteFrame {
        // avatar = "999";
        let avatarInt = parseInt(avatar) - 1;
        // if (avatarInt == 999) {
    ...
    loadScene(sceneName: string) {
        Global.BundleLobby.loadScene(sceneName,(finish: number, total: number) =>{
           this.showErrLoading(App.instance.getTextLang('txt_loading1') + (Utils.floatToInt((finish / total) * 100)).toString() + "%");
        },(err , sceneAssets) => {
            if(err){
                this.showErrLoading("បាត់បង់ការតភ្ជាប់ ស្នើសុំឱ្យភ្ជាប់ឡើងវិញ។!");
                return;
            }
            cc.director.runScene(sceneAssets);
        });
    ...
    actLoginCMD(isPlayNow = false) {
        ...
        Http.get(App.API_CMD, { t: "bl", nn: Configs.Login.Nickname, at: Configs.Login.AccessToken }, (err, res) => {
            ...
            if (res["code"] == 0) {
                ...
                this.ShowAlertDialog("ហ្គេមនេះស្ថិតនៅក្រោមការថែទាំ");
            }
        });
    }
                    ...
                    Http.get(App.API_IBC, { t: "Login", nn: Configs.Login.Nickname, at: Configs.Login.AccessToken }, (err, res) => {
                        ...
                        if (res["code"] == 0) {
                            if (Configs.App.IS_PRO == true && Configs.Login.UserType != "2") {
                                var url = "https://vietkieu888.com/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                if (cc.sys.isMobile == true) {
                                    url = "https://vietkieu888.com/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                }
                                cc.sys.openURL(url);

                            }
                            else {
                                var url = "http://vietkieu888.com/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                if (cc.sys.isMobile == true) {
                                    url = "http://vietkieu888.com/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                }
                                cc.sys.openURL(url);
                            }
                        }
                ...
                if (res["data"] == null) {
                    ...
                    if (balance < 10000 && isPlayNow == false) {
                        ...
                        Http.get(App.API_EBET, { t: "Login", nn: Configs.Login.Nickname, at: Configs.Login.AccessToken }, (err, res) => {
                            App.instance.showLoading(false);
                            if (res["res"] == 0) {
                                let url = "https://vietkieu888.com/h5/72895c?username=%s&accessToken=%s";
    ...
    checkTimeLixi() {
        ...
        let msg = "ព្រឹត្តិការណ៍ \"Golden Hour Lucky Draw\" នឹងប្រព្រឹត្តទៅនៅពេលក្រោយ៖ %sh %s ក្នុងរយៈពេលប៉ុន្មាននាទី!";
            ...
            if (err) {
             ...
             //   cc.log(res);
                if (res['data'] == "Not passed conditions") {
                    App.instance.ShowAlertDialog("អ្នកមិនមានសិទ្ធិចូលរួមក្នុងព្រឹត្តិការណ៍ \"Golden Hour Lucky Draw\"\nសូមអានច្បាប់ ឬទាក់ទងសេវាកម្មអតិថិជន!");
