    ...
    export class PopupUpdateNickname extends Dialog {
        ...
        show() {
            ...
            if (cc.sys.isBrowser) {
                ...
                // let url = "https://play.vietkieu888.com/?aff=NhungNgao4";
                if (url.includes("aff=")) {
        ...
        public actUpdate() {
            ...
            let checkUpdateNickName = nickname.toLowerCase().substring(0, 5);
            if(checkUpdateNickName === "guest"){
                App.instance.alertDialog.showMsg(App.instance.getTextLang("txt_update_check_guest_username_error"));
                return;
            }
            if(!Utils.isNumberIsChar(nickname)){
            ...
            if (this.at == "") {
                ...
                Http.get(Configs.App.API, { "c": 5, "un": _this.username, "pw": _this.password, "nn": nickname, "inv": inviteCode }, (err, res) => {
                    ...
                    if (!res["success"]) {
                        switch (parseInt(res["errorCode"])) {
                            case 1001:
                                App.instance.alertDialog.showMsg("បាត់បង់ការតភ្ជាប់ទៅម៉ាស៊ីនមេ!");
                                break;
                            case 1005:
                                App.instance.alertDialog.showMsg("គណនីមិនមានទេ។.");
                                break;
                            case 1007:
                                App.instance.alertDialog.showMsg("ពាក្យសម្ងាត់មិនត្រឹមត្រូវ.");
                                break;
                            case 1109:
                                App.instance.alertDialog.showMsg("គណនីត្រូវបានចាក់សោ.");
                                break;
                            case 106:
                                App.instance.alertDialog.showMsg("ឈ្មោះបង្ហាញមិនត្រឹមត្រូវ។");
                                break;
                            case 1010:
                            case 1013:
                                App.instance.alertDialog.showMsg("ឈ្មោះបង្ហាញមានរួចហើយ.");
                                break;
                            case 1011:
                                App.instance.alertDialog.showMsg("ឈ្មោះដែលបង្ហាញមិនអាចដូចគ្នានឹងឈ្មោះចូលទេ។.");
                                break;
                            case 116:
                                App.instance.alertDialog.showMsg("កុំជ្រើសរើសឈ្មោះដែលបង្ហាញរសើប។");
                                break;
                            case 1114:
                                App.instance.alertDialog.showMsg("ប្រព័ន្ធគឺជាការថែទាំ។ សូមត្រលប់មកវិញនៅពេលក្រោយ!");
                                break;
                            default:
                                App.instance.alertDialog.showMsg("មាន​បញ្ហា​បាន​កើតឡើង​សូម​ព្យាយាម​ម្ដងទៀត​នៅ​ពេលក្រោយ!");
                                break;
                        }
                        return;
                    }
